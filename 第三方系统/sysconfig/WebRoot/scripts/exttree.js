...
*/
function checkboxtree(id,type,treejson,treeurl,functions,checkModel,onlyLeafCheckable){
		this.tree=null;
		this.type=type;
		...
		checkboxtree.prototype.tree=function(){
			return this.tree;
		}
}
/*
		...
		Ext.onReady(function(){
			...
			for (var i=0;i<this.treejson.length;i++){
				var tree=new Ext.tree.TreeNode({
			      ...
			      if(treejson[i].children)toaddtree(tree,this.treejson[i].children);
			}
...
*		functions点击树的节点是执行的动作
*/
function customtree(id,type,treejson,treeurl,functions){
		this.tree=null;
		this.treejson=treejson;
		this.treeurl=treeurl;
		this.type=type;
